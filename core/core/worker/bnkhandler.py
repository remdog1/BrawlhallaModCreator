...
class BnkHandler:
    def __init__(self):
        ...
        bundled_wwiseutil_path = _resourcePath("tools", "wwiseutil", "wwiseutil.exe")
        cache_wwiseutil_path = os.path.join(self.tools_dir, "wwiseutil", "wwiseutil.exe")
        if os.path.exists(bundled_wwiseutil_path):
            self.wwiser_path = bundled_wwiseutil_path
        else:
            self.wwiser_path = cache_wwiseutil_path

        if not os.path.exists(self.wwiser_path):
            self.download_wwiser()

    def download_wwiser(self) -> bool:
        """Download and set up the wwiser tool. Uses certifi for SSL when available."""
        ...
        try:
            ...
            response = requests.get(release_url, timeout=30, verify=verify_ssl)
            ...
            response = requests.get(download_url, timeout=60, verify=verify_ssl)
            response.raise_for_status()
            ...
            SendNotification(
                ...
                "SSL error downloading wwiseutil (certificate verify failed). "
                "If you use a firewall, proxy, or corporate network, try from another network or add an exception. "
                "Details: " + str(e)
            )
            return False
        ...
        try:
            ...
            with open(os.path.join(log_dir, "wem_operations.log"), "a", encoding="utf-8") as log:
                ...
                try:
                    ...
                    # Extract game BNK file
                    game_bnk_path = BRAWLHALLA_FILES[target_file]
                    if not os.path.exists(game_bnk_path):
                        log.write(f"❌ Game BNK file does not exist: {game_bnk_path}\n")
                        return False
                    
                    # Extract game BNK to temp directory
                    extract_cmd = [
                        self.wwiser_path,
                        "-l", "english",
                        "-o", game_temp_dir,
                    ...
                    log.write(f"Extracting game BNK file...\n")
                    ...
                    # Create a mapping of WEM IDs that are already modified by other mods
                    protected_wem_ids = set()
                    for other_mod_hash, other_changes in self.mod_changes.items():
                        if other_mod_hash != mod_hash and target_file in other_changes:
                    ...
                    rebuild_result = subprocess.run(rebuild_cmd, capture_output=True, text=True, timeout=300)
                    ...
                    verify_cmd = [
                        self.wwiser_path,
                        "-l", "english",
                        "-o", os.path.join(self.cache_dir, "verify"),
                        game_bnk_path
                    ]
                    ...
                    if verify_result.returncode == 0:
                        rebuilt_wem_info = self.extract_wem_info(verify_result.stdout)
                        log.write(f"Verification successful - Found {len(rebuilt_wem_info)} WEM files\n")
                    else:
                        log.write(f"⚠️ Verification failed, but continuing\n")
                    ...
                    verify_dir = os.path.join(self.cache_dir, "verify")
