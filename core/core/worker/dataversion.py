...
class DataClass(metaclass=DataMetaclass):
    ...
    def _getDefaultValue(self, varName):
        for baseClass in self.__class__.__mro__:
            if varName in getattr(baseClass, "__dict__", {}):
                value = getattr(baseClass, varName)
                if not callable(value):
                    return copy.deepcopy(value)

            annotation = getattr(baseClass, "__annotations__", {}).get(varName)
    ...
    def ensureDataDefaults(self, ignoredVars=None):
        ...
        for varName in DataVariable.getVarNames(self.formatType, self.formatVersion):
            ...
            if not hasattr(self, varName) or getattr(self, varName) is None:
                setattr(self, varName, self._getDefaultValue(varName))
    ...
    def loadFromJson(self, json_: Union[str, dict], ignoredVars=None, allowedVars=None) -> bool:
        ...
        if formatVersion is not None:
            for varName in DataVariable.getVarNames(self.formatType, formatVersion):
                ...
                var = data.get(varName, self._getDefaultValue(varName))
                if var is None:
                    var = self._getDefaultValue(varName)
