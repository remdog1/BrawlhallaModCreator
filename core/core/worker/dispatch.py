...
class Dispatch(BaseDispatch):
    ...
    @Index(Environment.SetModFeatures)
    def setModFeatures(self, hash, features):
        print(f"DEBUG: Dispatch.setModFeatures called with hash={hash}, features={features}")
        modSources = ModLoader.getModSourcesByHash(hash)
        if modSources is not None:
            print(f"DEBUG: Found modSources, calling setFeatures")
            modSources.setFeatures(features)
            print(f"DEBUG: setFeatures completed, features now: {modSources.features}")
            return True, hash
        else:
            print(f"DEBUG: No modSources found for hash {hash}")
